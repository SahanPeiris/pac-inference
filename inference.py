    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                # print "self.beliefs[pos] is" + str(self.beliefs[pos])
                temp[newPos] += prob * self.beliefs[pos]

        self.beliefs = temp.copy()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #create global variable so that it can be accessed
        self.listOfParticles = listOfPositions
    ...
    def observe(self, observation, gameState):
        ...
        prevDistribution = self.getBeliefDistribution()
        for position in self.legalPositions:
            dist = util.manhattanDistance(position, pacmanPosition)
            #get new updated distribution
            tempDistribution[position] += emissionModel[dist] * prevDistribution[position]

        #case 2
        #if all particlaes recieve zero weight
        checkVal = sum(tempDistribution.values())
        if checkVal == 0:
            self.initializeUniformly(gameState)
            return

        #otherwise we reinitialize
        self.listOfParticles = []
        i = 0
        while i < numberOfParticles:
            self.listOfParticles.append(util.sample(tempDistribution))
            i += 1
    ...
    def elapseTime(self, gameState):
        ...
        self.listOfParticles = tmpParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.listOfParticles = positionList
    ...
    def observeState(self, gameState):
        ...
        #iterate through each particle and ghost
        for particle in self.listOfParticles:
            weight = 1.0
            for ghost in range(numberOfGhosts):
                #check if Noisy distance is zero, Otherwise set new weight of particle
                if noisyDistances[ghost] == None:
                    ...
                    newWeight = emissionModels[ghost][distance]
                    weight = weight * newWeight
            tempDistribution[particle] += weight
